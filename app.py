...
# ══════════════════════════════════════════════════════════════════
BG_PRIMARY      = "#F6F7F4"   # ivory tenue (warm neutral)
BG_SECONDARY    = "#FFFFFF"
BG_CARD         = "#FFFFFF"
TEXT_PRIMARY    = "#1A202C"
...
COLOR_ACCENT    = "#2C5282"
BORDER_COLOR    = "#E8EAEF"   # bordo piu morbido
CHART_COLORS    = ["#1A365D", "#2C5282", "#3182CE", "#4299E1", "#63B3ED", "#90CDF4", "#BEE3F8", "#E6F2FF"]

COLOR_GREEN     = "#38A169"
COLOR_RED       = "#E53E3E"
COLOR_GOLD      = "#C9A227"   # oro piu ricco
BG_CARD_HOVER   = "#F7FAFC"
BORDER_RADIUS   = "14px"     # raggio piu morbido
...
.kpi-tile {{
    ...
    position: relative;
    overflow: hidden;
    box-shadow: 0 1px 3px rgba(16,24,40,0.04), 0 6px 18px rgba(16,24,40,0.05);
...
div.stDownloadButton > button {{
    ...
    border: 1px solid {COLOR_HIGHLIGHT} !important;
    border-radius: {BORDER_RADIUS} !important;
    font-weight: 600 !important;
}}
...
# ══════════════════════════════════════════════════════════════════
# Seed globale: rende riproducibili Montecarlo e proiezione GBM tra un run
# e l'altro (utile per output mostrati a un cliente). Rimuovi/cambia questo
# valore se vuoi estrazioni casuali diverse a ogni esecuzione.
RANDOM_SEED = 42
...
def render_topbar(title: str, subtitle: str, data_loaded: bool = False):
    ...
    <div style="display:flex; align-items:flex-end; justify-content:space-between;
                padding:1rem 0 1.8rem 0; border-bottom:1px solid {BORDER_COLOR};
                margin-bottom:2rem;">
        <div>
...
def get_data_yahoo(ticker, start_dt):
    try:
        df = yf.download(ticker, start=start_dt, progress=False, auto_adjust=True)
        if not df.empty:
...
def fetch_historical_data(tickers_input, years, timeframe="Giornaliero"):
    ...
    for t in tickers_input:
        ...
        try:
            df = yf.download(t, start=start_date, interval=yf_interval, progress=False, auto_adjust=True)
            if not df.empty:
                col = 'Adj Close' if 'Adj Close' in df.columns else 'Close'
                series = df[col].squeeze()
                if isinstance(series, pd.Series): 
...
def infer_ann_factor(index) -> int:
    """Deriva il fattore di annualizzazione dallo spacing REALE dei dati,
    invece di dedurlo dalla label di frequenza scelta in sidebar.
    Questo evita di annualizzare rendimenti mensili con 252 (e simili)
    quando il timeframe scaricato e la frequenza di ribilanciamento non
    coincidono."""
    if len(index) < 2:
        return 252
    diffs = np.diff(np.asarray(index.values, dtype='datetime64[D]')).astype(float)
    median_days = float(np.median(diffs)) if len(diffs) else 1.0
    if median_days <= 3:      # giornaliero (salti weekend inclusi)
        return 252
    elif median_days <= 10:   # settimanale
        return 52
    else:                     # mensile
        return 12
...
def prep_data(df, assets, lookback, freq):
    ...
    # 'M' è deprecato in pandas >= 2.2 → mappa a 'ME' mantenendo la label UI invariata
    resample_freq = {"D": "D", "W": "W", "M": "ME"}.get(freq, freq)
    df_res = df.resample(resample_freq).last().dropna()
    returns = df_res.pct_change().dropna()
    if returns.empty: return None, None, "Impossibile calcolare rendimenti."
    # Annualizzazione robusta: dedotta dallo spacing effettivo, non dalla label freq
    ann_factor = infer_ann_factor(df_res.index)
...
with st.sidebar:
    st.markdown(f"""
    <div style="padding:0.8rem 0 1.8rem 0; border-bottom:1px solid {BORDER_COLOR}; margin-bottom:1.8rem;">
    ...
    st.markdown(f"""
    <div style="margin-top:2rem; padding-top:1rem; border-top:1px solid {BORDER_COLOR};">
...
if not st.session_state.data_loaded:
    ...
    <div style="text-align:center; padding:6rem 2rem;">
        ...
        <div style="margin-top:3rem; padding:2rem; background:{BG_CARD}; 
                    border:1px solid {BORDER_COLOR}; border-radius:{BORDER_RADIUS};
...
<div style="background:{BG_CARD}; padding:1.2rem 1.5rem; border-radius:{BORDER_RADIUS};
            border:1px solid {BORDER_COLOR}; margin-bottom:2rem;">
...
with tab5:
    ...
    if len(returns_wf) <= window_size:
        ...
        for col in df_wf.columns:
            ...
            # Rendimento ANNUALIZZATO (CAGR), coerente con la volatilità annualizzata.
            # Lo Sharpe usa questo, non il return cumulato, altrimenti le unità non tornano.
            ret_ann = (1 + ret_tot) ** (ann_factor_opt / n_periods) - 1 if n_periods > 0 else 0.0
            dd_max = max_drawdown(nav[col])
            vol = df_wf[col].std() * np.sqrt(ann_factor_opt)
            sharpe = (ret_ann - rf) / vol if vol > 0 else 0
            results_list.append({
                "Modello": col,
                "Return Totale": f"{ret_tot*100:.2f}%",
                "Return Annuo (CAGR)": f"{ret_ann*100:.2f}%",
